...
public class DAY1_manager : MonoBehaviour
{
    ...
    public float teacherMoveSpeed = 2f;
    ...
    void Start()
    {
        if (teacher != null)
        {
            ...
            {
                Debug.LogWarning("Teacher 오브젝트에 Animator 컴포넌트가 없습니다.", this.gameObject);
            }
        }
    ...
    void StartTeacherSequence()
    {
        ...
        reinaStillLook.SetLookSprite(false);
    ...
    IEnumerator MoveTeacher(Transform targetPos, System.Action onCompleted)
    {
        if (teacherAnimator != null)
        {
            float directionX = Mathf.Sign(targetPos.position.x - teacher.transform.position.x);
            teacherAnimator.SetFloat("moveX", directionX);
            teacherAnimator.SetFloat("speed", teacherMoveSpeed);
        }
        ...
        if (teacherAnimator != null)
        {
            teacherAnimator.SetFloat("speed", 0f);
        }
